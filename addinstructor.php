...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty ($fname_error) and empty ($email_error) and empty ($pwd_error) and empty ($gender_error)) {
        ...
        $target = "instructor/" . basename($image);
        move_uploaded_file($_FILES['exampleFormControlFile1']['tmp_name'], $target);

        $query = "INSERT INTO accounts(name, email, gender, password, role, profile) VALUES (?,?,?,?,?,?)";

        $stmt = mysqli_prepare($conn, $query);

        if ($stmt) {

            mysqli_stmt_bind_param($stmt, "ssssss", $p_name, $p_email, $p_gender, $p_password, $p_role, $p_profile);
    ...
    <main class="container">
        <div class="container card" style="max-width: 720px;">
            <form class="p-5" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <label for="inputGender" class="form-label">Gender&nbsp;<span class="form-text text-danger">
                ...
                <div class="mb-3">
                    <label for="exampleFormControlFile1" class="form-label">Profile Picture</label>
